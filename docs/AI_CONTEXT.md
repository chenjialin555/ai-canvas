...
> **维护**：重大架构或阶段完成时更新本文；目录全表、数据模型细节见 `doc/PROJECT_ARCHITECTURE.md`；去重规则见 [`DOC_MAP.md`](DOC_MAP.md)。
...
**完整树与文件职责表**：见 [`doc/PROJECT_ARCHITECTURE.md`](../doc/PROJECT_ARCHITECTURE.md) §3、§18（**勿在此复制**）。

硬事实摘要：

- 入口：`src/main.tsx` → `./app/App`（**无**根目录 `src/App.tsx`）
- 壳：`src/app/`；画布：`StageCanvas` + `src/canvas/`；状态：`src/editor/store/`
- AI：`src/ai/`（HTTP）+ `src/workflow/`（节点定义）
- 图片工具：`src/image-tools/`（主路径 `ImageEditorModal` + `*Panel`）
- 后端：`backend/app/`；归档：`src/_archive/`（exclude，不参与构建）
...
与 `docs/ROADMAP.md` Phase 1～9 对齐（摘要）：
...
| image-tools | `ImageEditorModal`、`CropEditorPanel`、`MaskEditorPanel`、`Panorama3DEditorPanel`、`maskRasterize`；`CropEditorModal`/`MaskEditorModal` 仍保留 |
| 扩展指南 | `docs/guides/*.md` 四篇 |
| Command / 历史 | `editor/commands/*`；**手势内** `commitHistory` 引用计数（多选 Transformer 不重复入栈）；undo 仍为 **全量快照** |

**入口**：`src/main.tsx` → `import App from "./app/App"`；依次引入 `editor.css`、`themes.css`、`skin-intuly.css`、`theme-bridge.css`、`pro-tool-ui.css`，并调用 `initUiTheme()`。
...
| `src/app/App.tsx` | 编排 ContextMenu、CanvasArea、快捷键副作用与 store；`openImageEditor` |
| `src/image-tools/ImageEditorModal.tsx` | 统一图片编辑外壳（裁剪 / 蒙版 / 解析3D Tab） |
| `src/image-tools/crop/CropEditorPanel.tsx` | 裁剪 Konva 子场景（主路径） |
| `src/image-tools/mask/MaskEditorPanel.tsx` | 蒙版笔刷（主路径） |
| `src/image-tools/panorama3d/Panorama3DEditorPanel.tsx` | Pannellum 全景预览（只读） |
| `src/lib/pannellumViewer.ts` | `window.pannellum` 封装 |
| `src/styles/pro-tool-ui.css` | 深色专业工具 UI 覆盖层 |
...
- [ ] 是否影响画布坐标或 API 契约？
- [ ] 是否需要同步 `docs/ROADMAP.md` 或本文？

---

## 16. 改代码后测试清单（最低限度）

见 [`guides/testing.md`](guides/testing.md)（命令 + 手动回归清单，**不在此重复**）。
...
| 日期 | 内容 |
|------|------|
| 2026-05-13 | 阶段 1～9 按 ROADMAP Phase 1～9 落实；含 app 壳、ai、workflow API、image-tools、guides、commands 等 |
| 2026-05-16 | 统一图片编辑器（裁剪/蒙版/解析3D Tab）、快捷条 `parse3d`、Pannellum、`pro-tool-ui` 深色 UI、`themes.css` Token；右键菜单去掉与快捷条重复的裁剪/AI/蒙版项 |
| 2026-05-17 | `RightSidebar` 属性栏剩余字段改走 `executeElementCommand`（`updateElement`）；连续编辑 `history: false` + `commitHistory`（数字/滑条/文本/颜色/滤镜）；组合「取消组合」走 `ungroupSelected` 命令；指南见 `docs/guides/element-command-system.md` |

（细粒度条目见 `docs/ROADMAP.md` 与 Git 历史。）
...
| 文档 | 用途 |
|------|------|
| [`docs/DOC_MAP.md`](DOC_MAP.md) | 主题唯一来源、去重规则 |
| `doc/PROJECT_ARCHITECTURE.md` | 全表、数据流、坐标、后端表 — **长参考** |
| `docs/guides/*.md` | 新增元素 / 节点 / Provider 等 how-to |
| `docs/MAINTENANCE_GUIDE.md` | **人类**维护白话手册 |
| `docs/ROADMAP.md` | 阶段 checklist |
