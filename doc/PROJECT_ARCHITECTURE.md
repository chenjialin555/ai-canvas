...
**相关短文档**：文档索引与去重 [`docs/DOC_MAP.md`](../docs/DOC_MAP.md)；AI 上下文 [`docs/AI_CONTEXT.md`](../docs/AI_CONTEXT.md)；维护手册 [`docs/MAINTENANCE_GUIDE.md`](../docs/MAINTENANCE_GUIDE.md)；阶段勾选 [`docs/ROADMAP.md`](../docs/ROADMAP.md)。  
**说明**：启动步骤、env 简表、概念导读以 `docs/guides/*` 与 `MAINTENANCE_GUIDE` 为准；**本文负责数据模型、坐标、OSS、源码全表等长参考**。
...
| nanoid | 5.x | id 生成 |
| pannellum | 2.x | 等距柱状全景预览（解析3D Tab，`lib/pannellumViewer` 封装） |
...
```text
src/
├── main.tsx                 → ./app/App；全局 CSS 链与 initUiTheme / initPlatform
...
├── image-tools/             # ImageEditorModal、裁剪/蒙版/解析3D Panel、legacy Modal、maskRasterize
├── ai/                      # api/、generation/、workflow/（单步生图与工作流 HTTP）
├── editor/                  # types、export、store/、commands/、quickTools（快捷条工具 id）
├── workflow/                # nodeRegistry、nodes/、utils/unifiedGraph
├── lib/                     # pannellumViewer（window.pannellum 封装）等
├── styles/                  # editor.css、themes.css、theme-bridge、skin-intuly、pro-tool-ui（深色工具 UI）
...
**近期产品/UI（2026-05）**：图片 **裁剪 / 蒙版 / 解析3D（等距柱状全景预览）** 由 `ImageEditorModal` 统一外壳 + Tab 切换；`FloatingToolbar` 含 `parse3d` 等工具（见 `editor/quickTools.ts`）。画布右键菜单已去掉与快捷条重复的「裁剪 / AI 生图 / 蒙版」项。全局主题为 `themes.css` 变量 + `pro-tool-ui.css` 专业深色覆盖层（`main.tsx` 按序引入）。
...
|-------------|------|-----------------|----------|
| `src/app/App.tsx` | 249 | `useEditorStore()`、`useAppModals` 等 | **应用编排**：`AppShell` 槽位、右键菜单、画布区回调、`EditorModals`；`openImageEditor(id, tool)` 打开统一图片编辑器 |
...
| `src/image-tools/ImageEditorModal.tsx` | 99 | `useAppModals` 的 `imageEditor` | **统一图片编辑**：Tab（裁剪 / 蒙版 / 解析3D）+ 子面板 |
| `src/image-tools/crop/CropEditorPanel.tsx` | 642 | `updateElement`、`clone` | 裁剪 Konva 子场景（嵌入统一编辑器）；`CropEditorModal` 仍为独立全屏版（少用） |
| `src/image-tools/mask/MaskEditorPanel.tsx` | 586 | `setImageAIMask` | AI 蒙版笔刷（嵌入统一编辑器）；`MaskEditorModal` 仍为独立全屏版 |
| `src/image-tools/panorama3d/Panorama3DEditorPanel.tsx` | 133 | 只读预览 | **Pannellum** 等距柱状全景；数据源为当前图片节点 `src` |
...
| `src/components/FloatingToolbar.tsx` | 315 | 选区、`marqueeSelecting`、`floatingToolbarSuppressed` | Portal 快捷条，世界 AABB → 屏幕坐标；含 **`parse3d`** 等工具 |
...
| `src/components/ContextMenu.tsx` | 112 | 右键回调 | 画布右键菜单；图片类「裁剪 / AI 生图 / 蒙版」已移除（改由浮动快捷条 + `ImageEditorModal`） |
...
sequenceDiagram
  ...
  participant Shell as ImageEditorModal
  participant Crop as CropEditorPanel
  participant Store as useEditorStore

  U->>App: 打开裁剪（快捷条 / 双击图等）
  App->>Shell: imageEditor = { id, tool: crop }
  Shell->>Crop: active tab
...
`MaskEditorPanel`（或独立 `MaskEditorModal`）在保存时组装 `ImageMaskData` → `setImageAIMask`。
...
```mermaid
flowchart TB
  A[选中 Image] --> B[ImageEditorModal · 蒙版 Tab]
  B --> C["setImageAIMask"]
  C --> D[AiGenerateModal]
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `src/main.tsx` | 19 | Vite + React 挂载；`initUiTheme()`；`editor.css` → `themes.css` → `skin-intuly.css` → `theme-bridge.css` → **`pro-tool-ui.css`** |
| `src/lib/pannellumViewer.ts` | 20 | 加载 `pannellum` UMD，封装 `createPannellumViewer`（Vite 兼容） |
...
| `src/components/ContextMenu.tsx` | 112 | 画布右键菜单（图片项不含裁剪/AI/蒙版，与快捷条分工） |
| `src/components/FloatingToolbar.tsx` | 315 | 选中元素上方 HTML 快捷工具条（含解析3D 等） |
...
### 18.2b `src/image-tools/`（统一图片编辑 + 裁剪 / 蒙版 / 解析3D）

| 文件 | 行数 | 职责 |
|------|------|------|
| `src/image-tools/types.ts` | 10 | `ImageEditorTool`：`crop` \| `mask` \| `parse3d` |
| `src/image-tools/ImageEditorModal.tsx` | 99 | `modal-mask` 外壳；顶栏 Tab；挂载各 Panel |
| `src/image-tools/crop/CropEditorPanel.tsx` | 642 | 裁剪 Konva 子场景（主路径） |
| `src/image-tools/crop/CropEditorModal.tsx` | 662 | 裁剪独立全屏 Modal（遗留/备用） |
| `src/image-tools/mask/MaskEditorPanel.tsx` | 586 | AI 蒙版笔刷（主路径） |
| `src/image-tools/mask/MaskEditorModal.tsx` | 604 | 蒙版独立全屏 Modal（遗留） |
| `src/image-tools/panorama3d/Panorama3DEditorPanel.tsx` | 133 | Pannellum 全景预览（只读，节点 `src`） |
| `src/image-tools/panorama3d/Panorama3DEditorModal.tsx` | 240 | 独立全景 Modal（遗留；主路径为 Panel） |
...
| `src/editor/quickTools.ts` | 198 | 快捷条作用域、工具 id 白名单与合并逻辑；含 **`parse3d`**（解析3D） |
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `src/app/App.tsx` | 249 | 编排入口：`AppShell`、`ContextMenu`、`CanvasArea` 回调、`EditorModals`、`openImageEditor` |
...
| `src/app/EditorModals.tsx` | 98 | 隐藏 file input、素材库、快捷条设置、单步生图、**`ImageEditorModal`**（统一图片编辑） |
| `src/app/hooks/useAppModals.ts` | 92 | Modal / 侧栏 tab / 右键菜单 / **`imageEditor: { imageId, tool }`** / toolbar toast / 替换图状态 |
